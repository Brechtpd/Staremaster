...
- **Worker configuration** – runtime metadata tracks desired worker counts and model priority per role. Supervisor spawns multiple workers with role-indexed IDs/models, Codex executors honour explicit model overrides, and the renderer exposes a configuration grid (counts + four-slot model priority) with "Apply", "Apply & start", and "Stop all" actions.
